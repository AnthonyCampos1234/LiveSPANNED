...
// Create and export the chart components we need
const AreaChart = ({
  data,
  index,
  categories,
  ...
  return (
    <ChartContainer config={config} className={cn(height, className)}>
      <RechartsPrimitive.AreaChart data={data}>
        ...
        {categories.map((category, idx) => (
          <RechartsPrimitive.Area
            ...
            type="monotone"
            dataKey={category}
            stroke={colors?.[idx] || undefined}
            fill={colors?.[idx] || undefined}
            fillOpacity={0.1}
...
const PieChart = ({
  ...
  return (
    <ChartContainer config={config} className={cn("h-80", className)}>
        ...
        >
          {data.map((entry, idx) => (
            <RechartsPrimitive.Cell
              key={`cell-${idx}`}
              fill={colors?.[idx % (colors?.length || 1)] || undefined}
            />
          ))}
        </RechartsPrimitive.Pie>
